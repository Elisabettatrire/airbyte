...
def get_state(state):
    ...
    if state:
        for state_message in state:
            parsed_message = AirbyteStateMessage.parse_obj(state_message)
            stream_state = parsed_message.stream.stream_state
            state_date = stream_state.state_date
            start_chunk_index = stream_state.start_chunk_index
